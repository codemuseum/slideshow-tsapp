...
Slideshow = new Class({
	...
	options: {
		...
		hu: '/',
		linked: false,
		loader: {'animate': ['/stylesheets/slideshow/loader-#.png', 12]},
		loop: true,
		match: /\?slide=(\d+)$/,
		overlap: true,
		paused: false,
...
*/
	...
	initialize: function(el, data, options){	
		...
		if (!data){
		  this.options.hu = ''; // use full urls for images and thumbnails
			data = {};
			var htmlThumbs = this.slideshow.getElements(this.classes.get('thumbnails') + ' img');
			var thumbIndex = 0;
			this.slideshow.getElements(this.classes.get('images') + ' img').each(function(img){
				var src = img.get('src'); // read full url
				var caption = img.get('alt') || img.get('title') || '';
				var href = img.getParent().get('href') || '';
				data[src] = {'caption': caption, 'href': href, 'thumbnail': htmlThumbs[thumbIndex++].get('src') };
			});
		}
...
window.addEvent('domready', function() { new Slideshow('slideshow', null, {controller: true, thumbnails:true, captions:true, delay: 3500 }); });